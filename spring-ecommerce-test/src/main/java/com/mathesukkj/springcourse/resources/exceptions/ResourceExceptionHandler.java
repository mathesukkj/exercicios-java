package com.mathesukkj.springcourse.resources.exceptions;
...
public class ResourceExceptionHandler {
    ...
    public ResponseEntity<StandardError> missingArguments(MissingArgumentsException e, HttpServletRequest req) {
        String error = "Missing arguments error.";
        HttpStatus status = HttpStatus.BAD_REQUEST;
        StandardError err = new StandardError(Instant.now(), status.value(), error, e.getMessage(),
                req.getRequestURI());
        return ResponseEntity.status(status).body(err);
