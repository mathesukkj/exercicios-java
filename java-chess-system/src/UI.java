...
public class UI {
    ...
    public static ChessPosition readChessPosition(Scanner s) {
        try {
            ...
            throw new InputMismatchException("Error reading ChessPosition. Valid positions go from a1 to h8");
        }
    }
    ...
    public static void printCapturedPieces(List<ChessPiece> captured) {
        List<ChessPiece> white = captured.stream()
                .filter(x -> x.getColor() == ColorEnum.WHITE)
                .collect(Collectors.toList());

        List<ChessPiece> black = captured.stream()
                .filter(x -> x.getColor() == ColorEnum.BLACK)
                .collect(Collectors.toList());
